        ...
        */
        ...
        var contentGenerator = function(d) {
            ...
            trowEnter.append("td")
                .classed("value",true)
                .html(function(p,i) { 
                  var ret = valueFormatter(p.value,i);
                  if(isNaN(p.value)) {
                    return "n/a";
                  }
                  return ret; 
            ...
            trowEnter.append("td")
                ...
                .html(function(p,i) { return getPrevYearTotal(d.value, p.key) });
            trowEnter.selectAll("td").filter(".previous").each(function(p) {
              var prev = d3.select(this).text();
              if(parseFloat(prev) > 0) {
                d3.select(this).classed("positive", true);
              } else {
                d3.select(this).classed("negative", true);
              }
            });
        ...
        var contentGeneratorLineChart = function(d) {
            if (content != null) return content;
            ...
            trowEnter.append("td")
                .classed("previous",true)
                .html(function(p,i) { return getPrevYearCountry(d.value, p.key) });
            trowEnter.selectAll("td").filter(".previous").each(function(p) {
              var prev = d3.select(this).text();
              if(parseFloat(prev) > 0) {
                d3.select(this).classed("positive", true);
        ...
        var contentGeneratorLineChartIndustry = function(d) {
            if (content != null) return content;
            ...
            trowEnter.append("td")
                .classed("previous",true)
                .html(function(p,i) { return getPrevYearIndustry(d.value, p.key) });
            trowEnter.selectAll("td").filter(".previous").each(function(p) {
              var prev = d3.select(this).text();
              if(parseFloat(prev) > 0) {
                d3.select(this).classed("positive", true);
        ...
        function nvtooltip() {
            ...
            var id = d3.select(chartContainer.parentNode).attr("id");
            var cg;

            switch(id) {
              case "chart_line_total":
                cg = contentGenerator;
                break;
              case "chart_line":
                cg = contentGeneratorLineChart;
                break;
              case "chart_line_industry":
                cg = contentGeneratorLineChartIndustry;
                break;
            }

            var left = position.left;
            var top = (fixedTop != null) ? fixedTop : position.top;
            var container = getTooltipContainer(cg(data));
    ...
    selection.each(function(data) {
      ...
      interactiveLayer.dispatch.on('elementMousemove', function(e) {
          ...
          if (allData.length > 2) {
            ...
            var id = d3.select(chart.container.parentNode.parentNode).attr("id");
            var svg = chart.container;

            if (indexToHighlight !== null) {
              highlightTotalLine(id, allData[indexToHighlight].key);
              allData[indexToHighlight].highlight = true;
            } else {
              highlightTotalLine(id, "none");
            }
          }
